...
void GlobalOptimization::GPS2XYZ(double latitude, double longitude, double altitude, double* xyz)
{
    ...
    if (ros::param::get("mirror_xy", mirror_xy))
    {
        if (mirror_xy)
        {
            latitude *= -1;
...
void GlobalOptimization::inputOdom(double t, Eigen::Vector3d OdomP, Eigen::Quaterniond OdomQ)
{
	...
	// adjusted global path
    pose_stamped.pose.position.x += offset_x;
	pose_stamped.pose.position.y += offset_y;
    pose_stamped.pose.position.z += offset_z;

    adjusted_global_path.header = pose_stamped.header;
    adjusted_global_path.poses.push_back(pose_stamped);

    // update last values
    lastP.x() = pose_stamped.pose.position.x;
    lastP.y() = pose_stamped.pose.position.y;
    lastP.z() = pose_stamped.pose.position.z;
...
void GlobalOptimization::updateGlobalPath()
{
    ...
    adjusted_global_path.poses.clear();

    if (ros::param::get("pub_ground_truth", pub_ground_truth))
    {
        if (pub_ground_truth)
        {
            offset_x = start_x - globalPoseMap.begin()->second[0];
            offset_y = start_y - globalPoseMap.begin()->second[1];
            offset_z = start_z - globalPoseMap.begin()->second[2];
        }
    }
